...
// 简约版初始化 Init method shorter version
...
// 将alertView 添加到传入的View之上  Show in specified view
...
// 将alertView 添加到window中的最上层View之上 Show in window
- (void)show
{
    UIView *superView;
    UIView *window = [[[UIApplication sharedApplication] delegate] window];
...
// 隐藏alertView  Hide and dismiss the alert
- (void)dismiss
{
...
- (void)calculateFrame
{
    ...
    if ( ! hasContentView ) {
        if ( ! hasModifiedFrame ){
            UIFont * titleFont = self.titleLabel.font ? self.titleLabel.font : [UIFont boldSystemFontOfSize:17];
            ...
            CGRect messageRect = [self.message boundingRectWithSize:maximumLabelSize
                                                            options:NSStringDrawingUsesLineFragmentOrigin
        ...
        } else if (!self.message || self.message.length == 0){
            titleLabelFrame = CGRectMake(self.messageLeftRightPadding, 0, self.width - self.messageLeftRightPadding * 2, self.height - self.buttonHeight);
        }else {
